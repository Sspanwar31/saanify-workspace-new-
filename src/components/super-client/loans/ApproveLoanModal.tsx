...
export function ApproveLoanModal({ isOpen, onClose, requestId }: ApproveLoanModalProps) {
  const { loanRequests, approveLoan } = useSuperClientStore()
  ...
  // Calculate limits
  const totalDeposits = request?.totalDeposits || 0
  const maxLimit = totalDeposits * 0.8
  ...
  // Auto-populate amount with requested amount
  useEffect(() => {
    if (request && isOpen) {
