...
export default function MoonHero({ heroConfig, scale, imageUrl, festivalKey }: Props) {
  const userScale = scale ?? heroConfig?.scale ?? 1.15;
  const posterUrl = heroConfig?.image_url || imageUrl;

  const rawKey = (festivalKey || heroConfig?.visual_key || heroConfig?.banner_visual_key || '').toUpperCase();
  const isKarwaChauth = rawKey.includes('KARWA') || rawKey.includes('CHAUTH') || rawKey.includes('SIEVE') || rawKey.includes('MOON_SIEVE');
  ...
  return (
    ...
    >
      ...
      {posterUrl ? (
        ...
        /* 🕌 EID: TRANSPARENT 3D GOLD MOSQUE & CRESCENT SKYLINE */
        <div className="relative z-10 w-full max-w-[280px] sm:max-w-[340px] flex items-center justify-center filter drop-shadow-[0_10px_30px_rgba(16,185,129,0.45)]">
          <svg viewBox="0 0 300 200" className="w-full h-auto overflow-visible">
            ...
            <g transform="translate(150, 32) rotate(-15)">
              <path d="M 12 -18 A 18 18 0 1 0 12 18 A 14 14 0 1 1 12 -18 Z" fill="url(#goldMoon)" filter="url(#goldGlow)" />
            </g>
            ...
            <text x="150" y="198" textAnchor="middle" fill="#FFD700" fontSize="18" fontWeight="900" fontFamily="Amiri, serif" filter="url(#goldGlow)">
              عيد مبارك
            </text>
          </svg>
        </div>
