...
const MASTER_PRESET_CONFIGS: Record<string, PresetConfig> = {
  // 🔥 LOHRI: Niche se UPAR ki taraf uthte hue Angarey
  LOHRI: { 
    default: { 
      gravity: -0.008, 
      spread: 0.8, 
  ...
  RAKSHA_BANDHAN:  { default: { gravity: 0.006, spread: 0.8, speed: 0.25, colors: DARK_COLORS.RAKSHA_BANDHAN, minSize: 3.5, maxSize: 7.5, maxCount: 250, glow: true, wobble: true, direction: 'downward', spawnY: -0.1 } },
  CHRISTMAS:       { default: { gravity: 0.005, spread: 0.6, speed: 0.2, colors: DARK_COLORS.CHRISTMAS, minSize: 5.5, maxSize: 12, maxCount: 220, glow: true, wobble: true, direction: 'downward', spawnY: -0.1 } },
  JANMASHTAMI:     { default: { gravity: 0.006, spread: 0.8, speed: 0.25, colors: DARK_COLORS.JANMASHTAMI, minSize: 3.5, maxSize: 8, maxCount: 260, glow: true, wobble: true, direction: 'downward', spawnY: -0.1 } },
  DUSSEHRA:        { default: { gravity: 0.006, spread: 0.7, speed: 0.35, maxCount: 300, minSize: 2, maxSize: 5, colors: DARK_COLORS.DUSSEHRA, glow: true, wobble: false, direction: 'downward', spawnY: -0.1 } },
  MAKAR_SANKRANTI: { default: { gravity: 0.004, spread: 0.8, speed: 0.15, colors: DARK_COLORS.MAKAR_SANKRANTI, minSize: 4.5, maxSize: 9, maxCount: 200, glow: true, wobble: true, direction: 'downward', spawnY: -0.1 } },
  NEW_YEAR:        { default: { gravity: 0.008, spread: 0.9, speed: 0.28, colors: DARK_COLORS.NEW_YEAR, minSize: 3.5, maxSize: 8, maxCount: 280, glow: true, wobble: true, direction: 'downward', spawnY: -0.1 } },
};
...
export default function ParticleEngine({ 
  ...
  useEffect(() => {
    ...
    // 🚀 SUPABASE DATABASE PROPS BINDING (Priority #1)
    const resolvedGravity = customGravity ?? heroConfig?.customGravity ?? activePreset.default.gravity ?? DEFAULT.gravity;
    const resolvedSpeed = customSpeed ?? (heroConfig?.customSpeed ? heroConfig.customSpeed / 2.5 : activePreset.default.speed) ?? DEFAULT.speed;
    const resolvedMinSize = customMinSize ?? heroConfig?.customMinSize ?? activePreset.default.minSize ?? DEFAULT.minSize;
    const resolvedMaxSize = customMaxSize ?? heroConfig?.customMaxSize ?? activePreset.default.maxSize ?? DEFAULT.maxSize;
    const resolvedMaxCount = customMaxCount ?? heroConfig?.customMaxCount ?? activePreset.default.maxCount ?? DEFAULT.maxCount;
    ...
    const spawn = (): Particle => {
      ...
      const speed = config.speed * rand(0.6, 1.0);
      ...
      return {
        ...
        vy: isUpward ? -Math.abs(speed * rand(0.9, 1.4)) : Math.abs(speed * rand(0.8, 1.2)),
        size,
        color: pick(getThemeColors()),
        life: rand(baseLife * 0.6, baseLife),
        maxLife: baseLife,
    ...
    const draw = (p: Particle) => {
      ...
      const fade = progress < 0.8 ? 1 : Math.max(0, 1 - (progress - 0.8) / 0.2);
      ...
      // 🔥 7. LOHRI: Natural Rising Fire Flame Ember
        ...
        // Organic Flame Shape (Bottom round, Top sharp)
        ctx.beginPath();
        ctx.moveTo(0, -s * 1.3);
    ...
    const animate = () => {
      ...
      particles.current = particles.current.filter((p) => {
        ...
        if (config.wobble) {
          p.vx += Math.sin(p.life * 0.03 + p.y * 0.003) * 0.035;
        }
