...
export default function DevDeepawaliCinematicIntro({ videoUrl, onComplete }: Props) {
  // Video Path in public/videos folder
  const videoSrc = videoUrl || '/videos/dev-deepawali-intro.mp4';
  ...
  // Load Google Fonts
  useEffect(() => {
    if (!document.getElementById('dev-deepawali-fonts-video')) {
      const link = document.createElement('link');
      link.id = 'dev-deepawali-fonts-video';
      link.href =
  ...
  // 🔊 SILENT BACKGROUND AUDIO UNMUTE (Plays audio in background)
  useEffect(() => {
    const enableAudio = () => {
      if (videoRef.current) {
