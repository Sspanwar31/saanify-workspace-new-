...
interface Kite {
  x: number; y: number; vx: number; vy: number; targetY: number; sz: number; d: number;
...
function mkKites(w:number,h:number):Kite[]{
  const k:Kite[]=[];
  for(let i=0;i<28;i++){
    const d=.12+Math.random()*.88,c=KC[Math.floor(Math.random()*KC.length)],dir=Math.random()>.5?1:-1;
    k.push({
      x:Math.random()*w,
      y:h + 100 + rand(50, 450),
      vx:(.12+Math.random()*.4)*d*dir,
      vy:-rand(1.5, 3.5) * d,
      targetY:h*.08+Math.random()*h*.38,
      sz:12+d*34,
      d,
      c1:c[0],c2:c[1],c3:c[2],
      fp:Math.random()*6.28,fs:3+Math.random()*4,sp:Math.random()*6.28,ss:.4+Math.random()*1.2,sa:6+Math.random()*18,
      bp:Math.random()*6.28,bs:.5+Math.random()*1,ba:3+Math.random()*10,
      ax:Math.random()*w,ay:h*.82+Math.random()*h*.18,at:2.0+Math.random()*3.5,
      on:false,hero:false
    });
  }
  return k.sort((a,b)=>a.d-b.d);
}
...
function drawGoldenGlow(ctx:CanvasRenderingContext2D,w:number,h:number,t:number){
  const p=eOC(cl((t-12.5)/.5,0,1));if(p<=0)return;const a=p*.22;
...
function drawFade(ctx:CanvasRenderingContext2D,w:number,h:number,t:number){
  const p=eIQ(cl((t-13.7)/.3,0,1));if(p<=0)return;
...
export default function MakarSankrantiCinematicIntro({ onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    const animate = (ts: number) => {
      ...
      // ─── TEXT ───
      if (t > 11.5) drawText(ctx, w, h, t, pe);
      ...
      if (t > 12.5 && ts - sp.trail > 40 && pe.count < 1400) {
