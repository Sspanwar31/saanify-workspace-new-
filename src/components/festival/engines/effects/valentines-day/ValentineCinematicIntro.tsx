...
export default function ValentineCinematicIntro({ videoUrl, onComplete }: Props) {
  // Video Path in public/videos folder
  const videoSrc = videoUrl || '/videos/valentines-day-intro.mp4';
  ...
  // Load Google Fonts
  useEffect(() => {
    if (!document.getElementById('valentine-google-fonts')) {
      const link = document.createElement('link');
      link.id = 'valentine-google-fonts';
      link.href =
  ...
  // Show 3D Gold Text Greeting for 5.5 seconds
  useEffect(() => {
    if (stage === 'greeting') {
  ...
  return (
    <div
      onClick={stage === 'video' ? enableAudio : undefined}
      className="fixed inset-0 z-[99999] bg-black/95 backdrop-blur-xl flex items-center justify-center p-3 sm:p-6 overflow-hidden select-none cursor-pointer"
    >
      {/* 🎬 STEP 1: SOLID 3D APP PLAYER CARD FRAME */}
      {stage === 'video' && (
        <div className="relative w-full max-w-6xl h-[82vh] md:h-[86vh] rounded-3xl overflow-hidden border-2 border-rose-500/40 shadow-[0_0_90px_rgba(244,63,94,0.25)] bg-black flex items-center justify-center">
      ...
      {/* 🏆 STEP 2: 3D GOLDEN GREETING TEXT */}
      <div
        className={`absolute inset-0 flex flex-col items-center justify-center text-center p-6 md:p-12 transition-all duration-1000 ease-out bg-gradient-to-b from-[#240212] via-black to-black z-40 ${
          stage === 'greeting'
      ...
      >
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[70vw] h-[70vw] max-w-[500px] max-h-[500px] bg-rose-500/20 rounded-full blur-[110px] pointer-events-none animate-pulse" />
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-48 h-1 bg-white/10 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-rose-500 to-amber-400 animate-[progress_5.5s_linear_forwards]" />
        </div>
      </div>
