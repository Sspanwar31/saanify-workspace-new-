...
export default function DussehraCinematicIntro({ videoUrl, onComplete }: Props) {
  // Video Path in public/videos folder
  const videoSrc = videoUrl || '/videos/dussehra-intro.mp4';
  ...
  // Load Google Fonts
  useEffect(() => {
    if (!document.getElementById('dussehra-google-fonts')) {
      const link = document.createElement('link');
      link.id = 'dussehra-google-fonts';
      link.href =
  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-black/95 backdrop-blur-xl flex items-center justify-center p-3 sm:p-6 overflow-hidden select-none cursor-pointer">
