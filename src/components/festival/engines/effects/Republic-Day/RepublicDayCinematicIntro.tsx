...
const DUR = 17.5; // Timed perfectly to complete at 17.5 seconds
...
export default function RepublicDayCinematicIntro({ onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      // FIX: Extended timeline boundary to 17.5s (Holds 3 seconds on pure white screen with Ashoka Chakra)
      if (t >= DUR) {
      ...
      // Climax Zoom: At t >= 11.5s, camera dollies deep inside the India Gate archway, making flag fill the screen
      const zoomClimax = t < 11.5 ? lerp(1.0, 1.10, eOE(t / 11.5)) : lerp(1.10, 2.5, clamp((t - 11.5) / 2.5, 0, 1));
      ...
      if (t >= 14.5) {
        const whiteFadeAlpha = clamp((t - 14.5) * 1.8, 0, 1); // 0.5s ultra smooth fade-to-white
        c.save();
