...
interface Particle {
  ...
  tp: number; rot: number; rs: number; on: boolean;
  turbOff: number; depth: number; confettiColor: number;
  sparklePhase: number; bokehRadius: number; bokehAperture: number;
}

interface Star {
  x: number; y: number; sz: number; br: number; tw: number; tp: number;
}

interface Bird {
  x: number; y: number; vx: number; vy: number;
  wingPhase: number; wingSpeed: number; sz: number;
  depth: number; flapAmplitude: number;
}

interface Sukhoi {
  x: number; y: number; vx: number; vy: number;
  trail: Array<{ x: number; y: number; a: number }>;
  afterburner: number; depth: number;
}

interface Camera {
  x: number; y: number; zoom: number; tilt: number;
  shakeX: number; shakeY: number; breathing: number;
  dollySpeed: number; targetZoom: number;
}

interface Lighting {
  torchIntensity: number; ambientIntensity: number; sunIntensity: number;
  sunAngle: number; sunColor: { r: number; g: number; b: number };
  backLightIntensity: number; rimLightIntensity: number;
  fogDensity: number; fogColor: { r: number; g: number; b: number };
  exposure: number;
}
...
interface FogLayer {
  ...
  speed: number; opacity: number; turbulence: number;
}
...
// ✅ FIX: Added Array.isArray guard to prevent crash
...
const RepublicDayCinematicIntro: React.FC<Props> = ({ onComplete }) => {
  ...
  const createParticle = (width: number, height: number): Particle => ({
    x: randomRange(0, width), y: randomRange(0, height),
    vx: randomRange(-0.5, 0.5), vy: randomRange(-0.5, 0.5),
    life: 0, ml: randomRange(60, 180), sz: randomRange(1, 4),
    r: randomInt(200, 255), g: randomInt(150, 220), b: randomInt(0, 50),
    a: randomRange(0.3, 0.8), tp: randomInt(0, 5),
    rot: randomRange(0, Math.PI * 2), rs: randomRange(-0.05, 0.05),
    on: false, turbOff: randomRange(0, Math.PI * 2),
    depth: randomRange(0.5, 1.5), confettiColor: randomInt(0, 2),
    sparklePhase: randomRange(0, Math.PI * 2),
    bokehRadius: randomRange(5, 20), bokehAperture: randomRange(3, 8)
  });
  ...
  const initializeFlagCloth = () => {
    ...
    for (let j = 0; j <= rows; j++) {
      for (let i = 0; i <= cols; i++) {
        points.push({ x: i * 8, y: j * 6, z: 0, vx: 0, vy: 0, vz: 0 });
      }
    }
    for (let j = 0; j <= rows; j++) {
      for (let i = 0; i <= cols; i++) {
        const idx = j * (cols + 1) + i;
        if (i < cols) constraints.push({ p1: idx, p2: idx + 1, length: 8 });
        if (j < rows) constraints.push({ p1: idx, p2: idx + (cols + 1), length: 6 });
      }
    }
  ...
  const initializeSystems = useCallback((width: number, height: number) => {
    ...
    for (let i = 0; i < STAR_COUNT; i++) {
      starsRef.current.push({
        x: randomRange(0, width), y: randomRange(0, height * 0.6),
        sz: randomRange(0.5, 2.5), br: randomRange(0.3, 1),
        tw: randomRange(0.5, 2), tp: randomRange(0, Math.PI * 2)
      });
    }
    ...
    for (let i = 0; i < BIRD_COUNT; i++) {
      birdsRef.current.push({
        x: randomRange(-200, width + 200), y: randomRange(height * 0.2, height * 0.5),
        vx: randomRange(0.5, 1.5), vy: randomRange(-0.2, 0.2),
        wingPhase: randomRange(0, Math.PI * 2), wingSpeed: randomRange(2, 4),
        sz: randomRange(3, 8), depth: randomRange(0.5, 1.5),
        flapAmplitude: randomRange(0.3, 0.8)
      });
    }

    sukhoisRef.current = [];
    for (let i = 0; i < SUKHOI_COUNT; i++) {
      sukhoisRef.current.push({
        x: -300 - i * 150, y: height * 0.15 + i * 40,
        vx: randomRange(3, 5), vy: randomRange(-0.1, 0.1),
        trail: [], afterburner: 1, depth: 0.8 + i * 0.1
      });
    }
  ...
  const updateFlagCloth = (time: number, windStrength: number) => {
    ...
    for (let iter = 0; iter < 5; iter++) {
      for (let c = 0; c < flag.constraints.length; c++) {
        const con = flag.constraints[c];
        const p1 = flag.points[con.p1];
        const p2 = flag.points[con.p2];
        const dx = p2.x - p1.x;
        const dy = p2.y - p1.y;
        const dz = p2.z - p1.z;
        const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);
        if (dist === 0) continue;
        const diff = (con.length - dist) / dist * 0.5;
        const offsetX = dx * diff;
        const offsetY = dy * diff;
        const offsetZ = dz * diff;
        if (con.p1 % (flag.cols + 1) !== 0) {
          p1.x -= offsetX; p1.y -= offsetY; p1.z -= offsetZ;
        }
        if (con.p2 % (flag.cols + 1) !== 0) {
  ...
  const updateCamera = (progress: number, time: number) => {
    ...
    if (progress < 0.167) {
      camera.x = 0; camera.y = 0; camera.zoom = 1; camera.tilt = 0;
    } else if (progress < 0.333) {
      const t = smoothstep(0.167, 0.333, progress);
      camera.x = lerp(0, 20, t); camera.y = lerp(0, -10, t);
      camera.zoom = lerp(1, 1.05, t); camera.tilt = lerp(0, -0.02, t);
    } else if (progress < 0.533) {
      const t = smoothstep(0.333, 0.533, progress);
      camera.x = lerp(20, 50, t); camera.y = lerp(-10, -20, t);
      camera.zoom = lerp(1.05, 1.1, t); camera.tilt = lerp(-0.02, -0.03, t);
    } else if (progress < 0.667) {
      const t = smoothstep(0.533, 0.667, progress);
      camera.x = lerp(50, 80, t); camera.y = lerp(-20, -15, t);
      camera.zoom = lerp(1.1, 1.15, t); camera.tilt = lerp(-0.03, -0.025, t);
    } else if (progress < 0.867) {
      const t = smoothstep(0.667, 0.867, progress);
      camera.x = lerp(80, 100, t); camera.y = lerp(-15, -5, t);
      camera.zoom = lerp(1.15, 1.2, t); camera.tilt = lerp(-0.025, -0.01, t);
    } else {
      const t = smoothstep(0.867, 1.0, progress);
      camera.x = lerp(100, 150, t); camera.y = lerp(-5, 0, t);
      camera.zoom = lerp(1.2, 2.0, t); camera.tilt = lerp(-0.01, 0, t);
    }
  ...
  const updateLighting = (progress: number) => {
    ...
    if (progress < 0.167) {
      l.torchIntensity = 0.8; l.ambientIntensity = 0.1; l.sunIntensity = 0;
      l.fogDensity = 0.4; l.fogColor = { r: 20, g: 30, b: 50 }; l.exposure = 0.7;
    } else if (progress < 0.333) {
      const t = smoothstep(0.167, 0.333, progress);
      l.torchIntensity = lerp(0.8, 1.0, t); l.ambientIntensity = lerp(0.1, 0.15, t);
      l.sunIntensity = 0; l.fogDensity = lerp(0.4, 0.35, t); l.exposure = lerp(0.7, 0.75, t);
    } else if (progress < 0.533) {
      const t = smoothstep(0.333, 0.533, progress);
      l.torchIntensity = lerp(1.0, 0.6, t); l.ambientIntensity = lerp(0.15, 0.3, t);
      l.sunIntensity = lerp(0, 0.4, t); l.sunAngle = lerp(-0.5, -0.3, t);
      l.sunColor = { r: 255, g: lerp(150, 200, t), b: lerp(50, 100, t) };
      l.fogDensity = lerp(0.35, 0.25, t);
      l.fogColor = { r: lerp(20, 60, t), g: lerp(30, 40, t), b: lerp(50, 50, t) };
      l.exposure = lerp(0.75, 0.85, t);
    } else if (progress < 0.667) {
      const t = smoothstep(0.533, 0.667, progress);
      l.torchIntensity = lerp(0.6, 0.3, t); l.ambientIntensity = lerp(0.3, 0.5, t);
      l.sunIntensity = lerp(0.4, 0.7, t); l.sunAngle = lerp(-0.3, -0.1, t);
      l.sunColor = { r: 255, g: lerp(200, 220, t), b: lerp(100, 150, t) };
      l.fogDensity = lerp(0.25, 0.2, t);
      l.fogColor = { r: lerp(60, 100, t), g: lerp(40, 60, t), b: lerp(50, 50, t) };
      l.exposure = lerp(0.85, 0.95, t);
    } else if (progress < 0.867) {
      const t = smoothstep(0.667, 0.867, progress);
      l.torchIntensity = lerp(0.3, 0.1, t); l.ambientIntensity = lerp(0.5, 0.7, t);
      l.sunIntensity = lerp(0.7, 1.0, t); l.sunAngle = lerp(-0.1, 0.1, t);
      l.sunColor = { r: 255, g: lerp(220, 240, t), b: lerp(150, 200, t) };
      l.fogDensity = lerp(0.2, 0.15, t);
      l.fogColor = { r: lerp(100, 140, t), g: lerp(60, 80, t), b: lerp(50, 60, t) };
      l.exposure = lerp(0.95, 1.0, t);
      l.backLightIntensity = lerp(0, 0.3, t); l.rimLightIntensity = lerp(0, 0.5, t);
    } else {
      const t = smoothstep(0.867, 1.0, progress);
      l.torchIntensity = lerp(0.1, 0, t); l.ambientIntensity = lerp(0.7, 0.8, t);
      l.sunIntensity = lerp(1.0, 1.2, t); l.sunAngle = lerp(0.1, 0.2, t);
      l.sunColor = { r: 255, g: lerp(240, 250, t), b: lerp(200, 230, t) };
      l.fogDensity = lerp(0.15, 0.1, t);
      l.fogColor = { r: lerp(140, 180, t), g: lerp(80, 100, t), b: lerp(60, 70, t) };
      l.exposure = lerp(1.0, 1.1, t);
      l.backLightIntensity = lerp(0.3, 0.6, t); l.rimLightIntensity = lerp(0.5, 0.8, t);
    }
  };
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (!p.on) {
        if (progress > 0.333 && p.tp === 0 && Math.random() < 0.02) {
          p.on = true; p.life = 0; p.x = randomRange(0, width); p.y = randomRange(height * 0.4, height * 0.9);
          p.vx = randomRange(-0.3, 0.3); p.vy = randomRange(-0.5, -0.1);
        } else if (progress > 0.533 && p.tp === 1 && Math.random() < 0.01) {
          p.on = true; p.life = 0; p.x = randomRange(width * 0.2, width * 0.8); p.y = height * 0.8;
  ...
  const updateSukhois = (progress: number, time: number, width: number, height: number) => {
    ...
    for (let i = 0; i < sukhois.length; i++) {
      const jet = sukhois[i];
      ...
      for (let j = 0; j < jet.trail.length; j++) { jet.trail[j].a *= 0.98; }
  ...
  const updateFogLayers = (progress: number, time: number, width: number) => {
    ...
    for (let i = 0; i < fogLayers.length; i++) {
      ...
      if (fog.x > width + fog.width) {
        fog.x = -fog.width; fog.y += randomRange(-20, 20);
      }
    }
  };
  ...
  const drawLensFlare = (ctx: CanvasRenderingContext2D, sunX: number, sunY: number, width: number, height: number, time: number, opacity: number) => {
    ...
    for (let i = 1; i <= 5; i++) {
      const t = i / 6;
      const x = lerp(sunX, centerX, t); const y = lerp(sunY, centerY, t);
  ...
  const drawIndiaGate = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number, time: number) => {
    ...
    ctx.translate(gateX, gateY); ctx.scale(gateScale, gateScale);
    ctx.transform(1, camera.tilt, 0, 1, 0, 0);

    const gateWidth = 300; const gateHeight = 200;
    const archWidth = 120; const archHeight = 150;
    const pillarWidth = 40; const pillarHeight = gateHeight + 30;
    const canopyWidth = gateWidth + 40; const canopyHeight = 30;
    ...
    ctx.beginPath(); ctx.ellipse(0, pillarHeight / 2 + 20, gateWidth * 0.7, 30, 0, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  ...
  const drawTorch = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number, time: number) => {
    ...
    ctx.save(); ctx.globalAlpha = visibility;
    ctx.translate(torchX, torchY); ctx.scale(torchScale, torchScale);
  ...
  const drawFlag = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number, time: number) => {
    ...
    const poleRiseProgress = smoothstep(0.333, 0.5, progress);
    const currentPoleHeight = (poleBaseY - poleTopY) * poleRiseProgress;
    const flagAttachY = poleBaseY - currentPoleHeight;

    const windStrength = 0.5 + flagOpenProgress * 1.5;
    updateFlagCloth(time, windStrength);

    const flag = flagRef.current;
    ctx.save(); ctx.translate(poleX, flagAttachY); ctx.scale(camera.zoom, camera.zoom);
  ...
  // ✅✅✅ MAIN FIX IS HERE — was passing wrong number of arguments before
  const drawAshokaChakra = (ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, time: number, opacity: number, lighting: Lighting) => {
    ...
    // ✅ FIX: Use ctx.createRadialGradient directly with correct 6 params
    // OLD (BROKEN): createRadialGradient(ctx, -radius*0.3, -radius*0.3, 0, 0, 0, radius, [...])
    //   → stops received `radius` (number) instead of array → .forEach crash!
    // NEW (FIXED): direct call with correct x0,y0,r0,x1,y1,r1 + stops
    const metalGradient = ctx.createRadialGradient(
      -radius * 0.3, -radius * 0.3, 0,   // inner circle (offset for specular)
      0, 0, radius                        // outer circle (center)
    );
    ...
    ctx.fillStyle = 'rgba(0, 0, 128, 0.9)'; ctx.fill();
    ctx.beginPath(); ctx.arc(0, 0, radius, 0, Math.PI * 2);
    ctx.strokeStyle = 'rgba(200, 200, 255, 0.8)'; ctx.lineWidth = 2; ctx.stroke();
    ctx.beginPath(); ctx.arc(-radius * 0.3, -radius * 0.3, radius * 0.15, 0, Math.PI * 2);
  ...
  const drawBirds = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number, time: number) => {
    ...
    for (let i = 0; i < birds.length; i++) {
      const bird = birds[i];
      const parallaxX = bird.x + camera.x * bird.depth;
      const parallaxY = bird.y + camera.y * bird.depth;
      const wingAngle = Math.sin(bird.wingPhase) * bird.flapAmplitude;
      ctx.save(); ctx.translate(parallaxX, parallaxY);
      ctx.strokeStyle = `rgba(30, 30, 30, ${0.7 * birdVisibility})`; ctx.lineWidth = 1.5; ctx.lineCap = 'round';
      ctx.beginPath();
      ctx.moveTo(-bird.sz, wingAngle * bird.sz);
      ctx.quadraticCurveTo(-bird.sz * 0.5, wingAngle * bird.sz * 0.5, 0, 0);
      ctx.quadraticCurveTo(bird.sz * 0.5, wingAngle * bird.sz * 0.5, bird.sz, wingAngle * bird.sz);
      ctx.stroke();
      ctx.restore();
    }
    ctx.restore();
  ...
  const drawSukhois = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number, time: number) => {
    ...
    for (let i = 0; i < sukhois.length; i++) {
      const jet = sukhois[i];
      const parallaxX = jet.x + camera.x * jet.depth;
      const parallaxY = jet.y + camera.y * jet.depth;

      // Draw trail
      if (jet.trail.length > 1) {
        for (let j = 1; j < jet.trail.length; j++) {
          const t0 = jet.trail[j - 1];
          const t1 = jet.trail[j];
          ctx.beginPath();
          ctx.moveTo(t0.x + camera.x * jet.depth, t0.y + camera.y * jet.depth);
          ctx.lineTo(t1.x + camera.x * jet.depth, t1.y + camera.y * jet.depth);
          ctx.strokeStyle = `rgba(200, 200, 255, ${t1.a * 0.3})`;
          ctx.lineWidth = 2 * t1.a; ctx.stroke();
        }
      }
      ...
      ctx.beginPath(); ctx.moveTo(-15, 0); ctx.lineTo(15, -3); ctx.lineTo(20, 0);
      ctx.lineTo(15, 3); ctx.closePath(); ctx.fill();
      // Wings
      ctx.beginPath(); ctx.moveTo(-5, 0); ctx.lineTo(0, -12); ctx.lineTo(5, 0);
      ctx.lineTo(0, 12); ctx.closePath(); ctx.fill();
      // Afterburner
      const abLength = 10 + jet.afterburner * 15;
      const abGradient = createGradient(ctx, -15, 0, -15 - abLength, 0, [
        { pos: 0, color: `rgba(100, 150, 255, ${jet.afterburner})` },
        { pos: 0.3, color: `rgba(150, 200, 255, ${jet.afterburner * 0.7})` },
        { pos: 1, color: `rgba(200, 220, 255, 0)` }
      ]);
      ctx.beginPath(); ctx.moveTo(-15, -2); ctx.lineTo(-15 - abLength, 0); ctx.lineTo(-15, 2); ctx.closePath();
      ctx.fillStyle = abGradient; ctx.fill();
      ctx.restore();
    }
    ctx.restore();
  };
  ...
  const drawParticles = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number, time: number) => {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      const px = p.x + camera.x * p.depth;
      const py = p.y + camera.y * p.depth;
      ...
      if (p.tp === 0) {
        ...
        // Ember
        const emberGlow = ctx.createRadialGradient(px, py, 0, px, py, p.sz * 3);
        emberGlow.addColorStop(0, `rgba(255, 150, 50, ${alpha})`);
        emberGlow.addColorStop(1, `rgba(255, 100, 0, 0)`);
        ctx.fillStyle = emberGlow;
        ctx.fillRect(px - p.sz * 3, py - p.sz * 3, p.sz * 6, p.sz * 6);
      } else if (p.tp === 2) {
        // Lens dust
        ctx.beginPath(); ctx.arc(px, py, p.sz * 0.5, 0, Math.PI * 2);
        ...
        ctx.fillStyle = colors[p.confettiColor];
        ctx.fillRect(-3, -2, 6, 4);
        ctx.restore();
      } else if (p.tp === 4) {
        // Sparkle
        const sparkle = Math.sin(p.sparklePhase) * 0.5 + 0.5;
        ctx.save(); ctx.globalAlpha = alpha * sparkle;
        ctx.translate(px, py); ctx.rotate(p.rot);
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(-1, -p.sz, 2, p.sz * 2);
        ctx.fillRect(-p.sz, -1, p.sz * 2, 2);
        ctx.restore();
      } else if (p.tp === 5) {
        // Bokeh
        ctx.beginPath(); ctx.arc(px, py, p.bokehRadius, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(255, 220, 150, ${alpha * 0.3})`;
        ctx.lineWidth = 1.5; ctx.stroke();
        ctx.beginPath(); ctx.arc(px, py, p.bokehAperture, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(255, 230, 180, ${alpha * 0.1})`; ctx.fill();
      }
    }
  };

  const drawVignette = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number) => {
    const vignetteStrength = 0.4 + (1 - smoothstep(0.167, 0.333, progress)) * 0.3;
  ...
  const drawTitle = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number, time: number) => {
    const titleOpacity = smoothstep(0.867, 0.92, progress);
    if (titleOpacity <= 0) return;
    ctx.save(); ctx.globalAlpha = titleOpacity;
    ...
    ctx.fillStyle = '#ffffff';
    ctx.shadowColor = 'rgba(255, 153, 51, 0.5)'; ctx.shadowBlur = 20;
    ctx.fillText('जय हिंद', width / 2, titleY);
    ctx.shadowBlur = 0;

    ctx.font = `300 ${fontSize * 0.4}px "Segoe UI", system-ui, sans-serif`;
    ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
    ctx.fillText('Republic Day — 26th January', width / 2, titleY + fontSize * 0.8);

    ctx.font = `200 ${fontSize * 0.3}px "Segoe UI", system-ui, sans-serif`;
    ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
    ctx.fillText('जय जवान, जय किसान', width / 2, titleY + fontSize * 1.3);
    ctx.restore();
  };

  const drawGround = (ctx: CanvasRenderingContext2D, width: number, height: number, progress: number) => {
    const groundY = height * 0.75;
    const lighting = lightingRef.current;
    const groundGradient = createGradient(ctx, 0, groundY, 0, height, [
      { pos: 0, color: `rgba(${60 * lighting.ambientIntensity + 80 * lighting.sunIntensity}, ${50 * lighting.ambientIntensity + 60 * lighting.sunIntensity}, ${30 * lighting.ambientIntensity + 30 * lighting.sunIntensity}, 1)` },
      { pos: 1, color: `rgba(${30 * lighting.ambientIntensity + 40 * lighting.sunIntensity}, ${25 * lighting.ambientIntensity + 30 * lighting.sunIntensity}, ${15 * lighting.ambientIntensity + 15 * lighting.sunIntensity}, 1)` }
    ]);
    ctx.fillStyle = groundGradient; ctx.fillRect(0, groundY, width, height - groundY);
  };
  ...
  const animate = useCallback((timestamp: number) => {
    ...
    updateParticles(progress, time, width, height);
    updateBirds(progress, time, width, height);
    updateSukhois(progress, time, width, height);
    updateFogLayers(progress, time, width);

    // Draw layers back to front
    drawSky(ctx, width, height, progress, time);
    drawFog(ctx, width, height, progress, time);
    drawGround(ctx, width, height, progress);
    drawIndiaGate(ctx, width, height, progress, time);
    drawTorch(ctx, width, height, progress, time);
    drawFlagPole(ctx, width, height, progress, time);
    drawFlag(ctx, width, height, progress, time);
    drawSukhois(ctx, width, height, progress, time);
    drawBirds(ctx, width, height, progress, time);
    drawParticles(ctx, width, height, progress, time);
    drawVignette(ctx, width, height, progress);
    drawTitle(ctx, width, height, progress, time);
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      // Re-init with CSS pixel dimensions
      initializeSystems(rect.width, rect.height);
    };
