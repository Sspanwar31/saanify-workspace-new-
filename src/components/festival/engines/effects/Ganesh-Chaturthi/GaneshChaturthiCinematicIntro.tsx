...
function buildGP():number[][]{
  const body:number[][][]=[
    [[200,58],[158,38],[78,48],[58,105]],[[58,105],[42,148],[68,178],[112,162]],
    [[112,162],[128,154],[140,162],[144,182]],[[144,182],[142,208],[126,226],[108,234]],
    [[108,234],[96,242],[90,254],[100,258]],[[100,258],[114,262],[124,250],[128,232]],
    [[128,232],[124,264],[116,314],[124,350]],[[124,350],[130,368],[158,386],[200,390]],
    [[200,390],[242,386],[270,368],[276,350]],[[276,350],[284,314],[276,264],[272,232]],
    [[272,232],[276,250],[286,262],[300,258]],[[300,258],[310,254],[304,242],[292,234]],
    [[292,234],[274,226],[258,208],[256,182]],[[256,182],[260,162],[272,154],[288,162]],
    [[288,162],[332,178],[358,148],[342,105]],[[342,105],[322,48],[242,38],[200,58]],
  ];
  const trunk:number[][][]=[
    [[190,152],[176,190],[142,240],[118,274]],[[118,274],[108,290],[94,294],[92,284]],
    [[92,284],[90,274],[100,266],[114,272]],
  ];
  const crown:number[][][]=[[[188,58],[192,32],[200,18],[208,32],[212,58]]];
  const all:number[][]=[];
  const add=(s:number[][][])=>{for(const c of s){const pts=ss(c[0],c[1],c[2],c[3],14);const s0=all.length>0?1:0;for(let i=s0;i<pts.length;i++)all.push(pts[i]);}};
...
export default function GaneshChaturthiCinematicIntro({onComplete}:Props){
  ...
  useEffect(()=>{
    ...
    function dOutline(t:number){
      ...
      /* wide glow */
      c.shadowColor='rgba(255,170,40,.5)';c.shadowBlur=18;
      c.strokeStyle=`rgba(255,190,65,.3)`;c.lineWidth=4;
      c.beginPath();c.moveTo(gp[0][0]*sc+ox,gp[0][1]*sc+oy);
      for(let i=1;i<show;i++)c.lineTo(gp[i][0]*sc+ox,gp[i][1]*sc+oy);c.stroke();
      /* bright core */
      c.shadowColor='rgba(255,210,85,.65)';c.shadowBlur=6;
      c.strokeStyle='rgba(255,222,125,.88)';c.lineWidth=1.7;
      c.beginPath();c.moveTo(gp[0][0]*sc+ox,gp[0][1]*sc+oy);
      for(let i=1;i<show;i++)c.lineTo(gp[i][0]*sc+ox,gp[i][1]*sc+oy);c.stroke();
      /* leading dot */
      if(t-ps<dur&&show>0){
