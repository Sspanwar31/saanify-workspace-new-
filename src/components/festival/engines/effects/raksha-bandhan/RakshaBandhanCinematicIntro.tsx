...
function genSilhouette(cx: number, cy: number, s: number, type: 'warrior' | 'brother' | 'sister'): { x: number; y: number }[] {
  ...
  ell(cx, cy - 72*s, 12*s, 14*s, 55);                          // head
  ell(cx, cy - 56*s, 5*s, 4*s, 10);                            // neck
  const sw = type === 'warrior' ? 50*s : 36*s;
  ell(cx, cy - 42*s, sw, 12*s, 60);                             // shoulders
  const mw = type === 'warrior' ? 26*s : 19*s;
  ell(cx, cy - 28*s, mw, 10*s, 45);                             // mid torso
  ell(cx, cy - 16*s, mw * 0.85, 8*s, 30);                      // waist
  const hw = type === 'warrior' ? 24*s : 18*s;
  ell(cx, cy - 5*s, hw, 7*s, 28);                               // hips
  ...
  if (type === 'sister') {
    ...
    ell(cx + sw + 17*s, cy - 96*s, 3*s, 22*s, 22);             // sword
  } else {
...
export default function RakshaBandhanCinematicIntro({ onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    const drawRakhiArt = (cx: number, cy: number, prog: number, detail: number, time: number) => {
      ...
      // Ambient glow
      const gr = ctx.createRadialGradient(cx, cy, R * 0.2, cx, cy, R * 2.8);
      gr.addColorStop(0, `rgba(255,215,0,${0.14 * prog})`);
      ...
      for (let i = 0; i < 12; i++) {
        ...
        const sx2 = cx + Math.cos(a) * R * 0.96;
        const sy2 = cy + Math.sin(a) * R * 0.96;
        const tl = R * 0.75 * prog;
        ctx.beginPath();
        ctx.strokeStyle = `rgba(255,180,0,${0.5 * prog})`;
        ctx.lineWidth = 1.2;
        ctx.shadowColor = C.gold; ctx.shadowBlur = 3;
        ctx.moveTo(sx2, sy2);
        for (let j = 1; j <= 8; j++) {
          const t2 = j / 8;
          ctx.lineTo(sx2 + Math.sin(t2 * 2.5 + time * 0.002 + i) * 3, sy2 + t2 * tl);
        }
        ctx.stroke();
      }
      ...
      ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI * 2 * Math.min(1, prog * 1.15)); ctx.stroke();

      // Inner ring
      ctx.strokeStyle = C.deepGold; ctx.lineWidth = 1.8; ctx.shadowBlur = 10;
      ctx.beginPath(); ctx.arc(cx, cy, R * 0.72, 0, Math.PI * 2 * Math.min(1, prog * 1.1)); ctx.stroke();

      // Dots
      if (prog > 0.4) {
        const dp = clamp((prog - 0.4) / 0.4, 0, 1);
        for (let i = 0; i < 16; i++) {
          const a = (i / 16) * Math.PI * 2 + time * 0.0002;
          ctx.fillStyle = `rgba(255,248,220,${dp * (0.4 + Math.sin(time * 0.004 + i) * 0.2)})`;
          ctx.shadowBlur = 6;
          ctx.beginPath();
          ctx.arc(cx + Math.cos(a) * R * 0.86, cy + Math.sin(a) * R * 0.86, 2 * sc, 0, Math.PI * 2);
          ctx.fill();
        }
      }
      ...
      if (detail > 0) {
        ...
        ctx.strokeStyle = `rgba(255,215,0,${detail * 0.5})`; ctx.lineWidth = 1.2;
        for (let i = 0; i < 8; i++) {
          const a = (i / 8) * Math.PI * 2 + time * 0.0003;
          ctx.beginPath();
        ...
        ctx.strokeStyle = `rgba(255,200,50,${detail * 0.35})`; ctx.lineWidth = 1;
        for (let i = 0; i < 6; i++) {
          const a1 = (i / 6) * Math.PI * 2;
          const a2 = ((i + 0.5) / 6) * Math.PI * 2;
          const am = (a1 + a2) / 2;
          ctx.beginPath();
          ctx.moveTo(cx + Math.cos(a1) * R * 0.28, cy + Math.sin(a1) * R * 0.28);
          ctx.quadraticCurveTo(cx + Math.cos(am) * R * 0.52, cy + Math.sin(am) * R * 0.52, cx + Math.cos(a2) * R * 0.28, cy + Math.sin(a2) * R * 0.28);
          ctx.stroke();
        }
      }
    ...
    const drawText = (prog: number, glow: number, time: number) => {
      ...
      ctx.fillStyle = `rgba(255,215,0,${prog * 0.92})`;
      ctx.fillText(txt, cx, cy + yOff);

      ctx.shadowBlur = (12 + glow * 22) * 0.4;
      ctx.fillStyle = `rgba(255,248,220,${prog * 0.65})`;
      ctx.fillText(txt, cx, cy + yOff);
