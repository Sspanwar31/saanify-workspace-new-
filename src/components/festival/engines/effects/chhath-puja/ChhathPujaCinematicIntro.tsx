...
export default function ChhathPujaCinematicIntro({ videoUrl, onComplete }: Props) {
  // Video Path in public/videos folder
  const videoSrc = videoUrl || '/videos/chhath-puja-intro.mp4';
  ...
  // Load Google Fonts
  useEffect(() => {
    if (!document.getElementById('chhath-google-fonts-video')) {
      const link = document.createElement('link');
      link.id = 'chhath-google-fonts-video';
      link.href =
  ...
  // Show 3D Gold Sanskrit Surya Mantra Text for 5.5 seconds
  useEffect(() => {
    if (stage === 'greeting') {
  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-black/95 backdrop-blur-xl flex items-center justify-center p-3 sm:p-6 overflow-hidden select-none">
      ...
      {/* 🏆 STEP 2: 3D GOLDEN DEVANAGARI SURYA MANTRA GREETING TEXT */}
      <div
        className={`absolute inset-0 flex flex-col items-center justify-center text-center p-6 md:p-12 transition-all duration-1000 ease-out bg-gradient-to-b from-[#2a1002] via-black to-black z-40 ${
          stage === 'greeting'
