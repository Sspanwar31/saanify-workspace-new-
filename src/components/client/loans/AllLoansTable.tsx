...
export function EditLoanModal({ isOpen, onClose, loanData }: EditLoanModalProps) {
  ...
  useEffect(() => {
    if (loanData) {
      // Ensure values are numbers to avoid errors
      setAmount(Number(loanData.amount) || 0);
      setRemainingBalance(Number(loanData.remainingBalance) || 0);
    }
  }, [loanData, isOpen]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // FIXED: Use router.refresh() instead of window.location.reload() for Next.js
      router.refresh();
