...
import { Download, DollarSign, TrendingUp, CreditCard, AlertTriangle, Calendar, Percent } from 'lucide-react';
...
export default function ReportsPage() {
        ...
        if (cid) {
            ...
            if (passbookRes.data) {
                ...
                const mappedPassbook = validEntries.map((e: any) => {
                    ...
                    return {
                        id: e.id, date: e.date, memberId: e.member_id,
                        type: type, description: desc, amount: total,
                        paymentMode: e.payment_mode || 'CASH', balance: runningBal
                    };
                });
                setPassbookEntries(mappedPassbook.reverse());
            }
        }
        setLoading(false);
    };
  ...
  // 2. CALCULATION ENGINE (UPDATED LOGIC)
  useEffect(() => {
    ...
    // --- LOGIC FIX 1: P&L Summary (Strict Type Checking) ---
    // Ab Expenses income me nahi judenge
    ...
    filteredExpenses.forEach(e => {
        ...
        // Only add to Other Income if explicitly marked as INCOME
        if (e.type === 'INCOME') {
            otherIncome += amt;
        }
        // Only add to Expense if explicitly marked as EXPENSE
        if (e.type === 'EXPENSE') {
            opsExpense += amt;
        }
    });
    ...
    const getOrSetEntry = (dateStr: string) => {
        if (!ledgerMap.has(dateStr)) {
            ledgerMap.set(dateStr, { 
                ...
                cashIn: 0, cashOut: 0, runningBal: 0, netFlow: 0,
    ...
    // --- LOGIC FIX 2: Member Report & NaN Fix ---
    // Safe calculation for Principal Paid
    const memberReports = members.map(m => {
        ...
        const lBal = mLoans.reduce((acc, l) => acc + (Number(l.remaining_balance) || 0), 0);
        const principalPaid = lTaken - lBal;
        
        return {
            id: m.id, name: m.name, fatherName: m.phone,
            totalDeposits: dep, loanTaken: lTaken, 
            principalPaid: principalPaid > 0 ? principalPaid : 0, 
            interestPaid: intPaid, finePaid: finePaid, activeLoanBal: lBal,
            netWorth: dep - lBal, status: m.status || 'active'
        };
    });

    // --- LOGIC FIX 3: Maturity Toggle (Auto vs Manual) ---
    // Check if DB has specific maturity amount
    const maturity = members.map(m => {
        ...
        const autoMaturity = target + projected;
        
        // CHECK MANUAL OVERRIDE (DB column 'maturity_amount')
        const finalMaturity = (Number(m.maturity_amount) > 0) ? Number(m.maturity_amount) : autoMaturity;
        ...
        return {
            ...
            outstandingLoan: outstanding, netPayable: finalMaturity - outstanding
        };
    });

    // --- LOGIC FIX 4: Defaulters (60 Days Rule) ---
    const defaulters = loans
        .filter(l => l.status === 'active' && Number(l.remaining_balance) > 0)
        .map(l => {
            // Calculate days overdue based on last payment or start date
            const lastPayment = rawPassbook
                .filter(p => p.member_id === l.member_id && (Number(p.installment_amount) > 0 || Number(p.interest_amount) > 0))
                .sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
            
            const referenceDate = lastPayment ? new Date(lastPayment.date) : new Date(l.start_date);
            const daysDiff = Math.floor((new Date().getTime() - referenceDate.getTime()) / (1000 * 3600 * 24));
        ...
        .filter(d => d.daysOverdue > 60); // Show only if overdue > 60 days
    ...
    setAuditData({
        ...
        loans: loans.map(l => ({ 
            ...l, 
            principalPaid: (Number(l.amount) || 0) - (Number(l.remaining_balance) || 0),
  ...
  const summary = {
    ...
    expenses: { ...auditData.summary.expenses, total: totalExpenses },
    netProfit
  };
  ...
  const enhancedDefaulters = auditData.defaulters.map((defaulter: any) => {
    ...
    const status = defaulter.daysOverdue > 90 ? 'Critical' : 'Overdue';
    return {
      ...defaulter,
  ...
  return (
    <div className="space-y-6 p-6">
      {/* Page Header and Global Filters remain exactly same */}
      ...
      <div className="border-b border-border/20 bg-white/80 dark:bg-black/60 backdrop-blur-sm -mx-6 px-6 py-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="relative">
            ...
            <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="pl-10 w-36 bg-white/50 dark:bg-black/20 border-white/20 dark:border-white/10" />
          </div>
          <div className="relative">
            <Calendar className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="pl-10 w-36 bg-white/50 dark:bg-black/20 border-white/20 dark:border-white/10" />
          </div>
          <Select value={selectedMember} onValueChange={setSelectedMember}>
            <SelectTrigger className="w-40 bg-white/50 dark:bg-black/20 border-white/20 dark:border-white/10"><SelectValue placeholder="All Members" /></SelectTrigger>
            <SelectContent><SelectItem value="ALL">All Members</SelectItem>{members.map(member => (<SelectItem key={member.id} value={member.id}>{member.name}</SelectItem>))}</SelectContent>
          </Select>
          <Select value={transactionMode} onValueChange={setTransactionMode}>
            <SelectTrigger className="w-40 bg-white/50 dark:bg-black/20 border-white/20 dark:border-white/10"><SelectValue placeholder="Transaction Mode" /></SelectTrigger>
            <SelectContent><SelectItem value="all">All Modes</SelectItem><SelectItem value="online">Online</SelectItem><SelectItem value="cash">Cash</SelectItem><SelectItem value="cheque">Cheque</SelectItem><SelectItem value="transfer">Bank Transfer</SelectItem></SelectContent>
          </Select>
          <Select value={transactionType} onValueChange={setTransactionType}>
            <SelectTrigger className="w-32 bg-white/50 dark:bg-black/20 border-white/20 dark:border-white/10"><SelectValue placeholder="Type" /></SelectTrigger>
            <SelectContent><SelectItem value="all">All Types</SelectItem><SelectItem value="loan">Loan</SelectItem><SelectItem value="deposit">Deposit</SelectItem><SelectItem value="payment">Payment</SelectItem><SelectItem value="expense">Expense</SelectItem></SelectContent>
          </Select>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 h-[350px]">
                ...
                <CardContent className="h-[280px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={graphData.incomeVsExpense}>
                            ...
                            <Bar dataKey="value" fill="#8884d8">{graphData.incomeVsExpense.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.fill} />))}</Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </CardContent>
            </Card>
            ...
            <Card className="p-4">
                ...
                <CardContent className="h-[280px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie data={graphData.paymentModes} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} outerRadius={80} fill="#8884d8" dataKey="value">
                                {graphData.paymentModes.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.fill} />))}
                            </Pie>
                            <Tooltip formatter={(value) => formatCurrency(Number(value))} />
                        </PieChart>
                    </ResponsiveContainer>
        ...
        <TabsContent value="daily" className="mt-6">
          <Card className="rounded-xl border-orange-100 shadow-sm">
            <CardHeader><CardTitle>Daily Ledger - The Operational Audit</CardTitle></CardHeader>
            <CardContent>
                  ...
                  <TableBody>
                    {auditData.dailyLedger.map((entry: any, i: number) => (
                      <TableRow key={i}>
                        ...
                        <TableCell className={entry.netFlow >= 0 ? 'text-green-600' : 'text-red-600'}>{formatCurrency(entry.netFlow || 0)}</TableCell>
        ...
        <TabsContent value="cashbook" className="space-y-6 mt-6">
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <Card className="rounded-xl border-orange-100 shadow-sm">
                ...
                <CardContent>
                    <div className="text-2xl font-bold text-orange-600">{formatCurrency((auditData.modeStats.cashBal || 0) + (auditData.modeStats.bankBal || 0) + (auditData.modeStats.upiBal || 0))}</div>
                    <p className="text-xs text-muted-foreground">All funds combined</p>
                </CardContent>
            </Card>
          </div>
          <Card className="rounded-xl border-orange-100 shadow-sm">
            <CardHeader><CardTitle>Cashbook - The Tally View</CardTitle></CardHeader>
            <CardContent>
              <div className="max-h-96 overflow-y-auto">
                <Table>
                  <TableHeader className="sticky top-0 bg-white">
        ...
        <TabsContent value="passbook" className="mt-6">
          <Card className="rounded-xl border-orange-100 shadow-sm">
            <CardHeader><CardTitle>Passbook Entries</CardTitle></CardHeader>
            <CardContent>
              <div className="max-h-96 overflow-y-auto">
                <Table>
                  <TableHeader className="sticky top-0 bg-white">
                    ...
                    {passbookEntries.map((entry: any, i: number) => {
                      ...
                      return (
                        <TableRow key={i}>
                          ...
                          <TableCell><Badge variant={entry.type === 'DEPOSIT' ? 'default' : 'secondary'}>{entry.type}</Badge></TableCell>
                          <TableCell>{entry.description}</TableCell>
                          <TableCell className={entry.type === 'DEPOSIT' ? 'text-green-600' : 'text-red-600'}>{fmt(Math.abs(entry.amount))}</TableCell>
        ...
        <TabsContent value="loans" className="space-y-6 mt-6">
          ...
          <Card className="rounded-xl border-orange-100 shadow-sm">
            <CardHeader><CardTitle>Loan Portfolio Details</CardTitle></CardHeader>
            <CardContent>
              <div className="max-h-96 overflow-y-auto">
                <Table>
                  <TableHeader className="sticky top-0 bg-white">
                    <TableRow>
                        ...
                        <TableHead>Balance</TableHead>
                        <TableHead>Interest Rate (1%)</TableHead>
                    ...
                    {auditData.loans.map((loan: any, i: number) => {
                      ...
                      return (
                        <TableRow key={i}>
                          ...
                          <TableCell className="text-green-600">{fmt(loan.principalPaid)}</TableCell>
                          <TableCell className="text-red-600 font-medium">{fmt(loan.remainingBalance)}</TableCell>
                          <TableCell>12% / 1% p.m.</TableCell>
                          <TableCell><Badge variant={loan.status === 'active' ? 'destructive' : loan.status === 'completed' ? 'default' : 'secondary'}>{loan.status}</Badge></TableCell>
                        </TableRow>
                      );
                    })}
                  </TableBody>
        ...
        <TabsContent value="members" className="mt-6">
          <Card className="rounded-xl border-orange-100 shadow-sm">
            <CardHeader><CardTitle>Members Report - Financial Health</CardTitle></CardHeader>
            <CardContent>
              <div className="max-h-96 overflow-y-auto">
                <Table>
                  <TableHeader className="sticky top-0 bg-white">
                  ...
                  <TableBody>
                    {auditData.memberReports.map((member: any, i: number) => (
                      <TableRow key={i}>
                        <TableCell className="font-medium"><div><div className="font-semibold">{member.name}</div><div className="text-xs text-muted-foreground">{member.fatherName}</div></div></TableCell>
                        ...
                        <TableCell className={`font-bold ${member.netWorth >= 0 ? 'text-green-600' : 'text-red-600'}`}>{fmt(member.netWorth)}</TableCell>
        ...
        <TabsContent value="maturity" className="mt-6">
          <Card className="rounded-xl border-orange-100 shadow-sm">
            <CardHeader><CardTitle>Maturity Projections - Fixed Columns</CardTitle></CardHeader>
            <CardContent>
              <div className="max-h-96 overflow-y-auto">
                <Table>
                  <TableHeader className="sticky top-0 bg-white">
                  ...
                  <TableBody>
                    {auditData.maturity.map((maturity: any, i: number) => (
                      <TableRow key={i}>
                        ...
                        <TableCell className={`font-medium ${maturity.netPayable >= 0 ? 'text-green-600' : 'text-red-600'}`}>{fmt(maturity.netPayable)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
        ...
        <TabsContent value="defaulters" className="mt-6">
          <Card className="rounded-xl border-orange-100 shadow-sm">
            <CardHeader><CardTitle>Defaulters List (Overdue {'>'} 60 Days)</CardTitle></CardHeader>
            <CardContent>
              <div className="max-h-96 overflow-y-auto">
                <Table>
                  <TableHeader className="sticky top-0 bg-white">
                  ...
                  <TableBody>
                    {enhancedDefaulters.map((defaulter: any, i: number) => (
                      <TableRow key={i}>
                        ...
                        <TableCell><Badge variant={defaulter.status === 'Critical' ? 'destructive' : 'secondary'}>{defaulter.status}</Badge></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
