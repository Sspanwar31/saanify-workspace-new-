...
import { 
  ...
  Lock, Unlock, Link as LinkIcon, Save, Check, X, AlertTriangle
} from 'lucide-react';
...
export default function UserManagementPage() {
  ...
  const handleToggleBlock = async (user: any) => {
    ...
    if (!error) setUsers(users.map(u => u.id === user.id ? { ...u, status: newStatus } : u));
  };

  // --- PERMISSION HANDLERS ---
  const togglePermission = (role: string, permission: string) => {
    if (!isEditingRoles) return;
    // Lock Admin to always have all
    if (role === 'client_admin') return; 
    
    setRoleConfig((prev: any) => {
      const currentPerms = prev[role];
      if (currentPerms.includes(permission)) {
        return { ...prev, [role]: currentPerms.filter((p: string) => p !== permission) };
      } else {
  ...
  const savePermissions = () => {
    // Here you would save to Supabase (e.g., a 'roles' table)
    // For now, it just saves to local state and toggles edit mode off
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="all-users" className="space-y-4">
            <div className="bg-white p-4 rounded-lg border flex flex-wrap gap-4 items-center">
                ...
                <Select value={filterRole} onValueChange={setFilterRole}>
                    ...
                    <SelectContent><SelectItem value="all">All Roles</SelectItem><SelectItem value="client_admin">Client Admin</SelectItem><SelectItem value="treasurer">Treasurer</SelectItem><SelectItem value="member">Member</SelectItem></SelectContent>
                </Select>
            </div>
            <Card><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader className="bg-gray-50"><TableRow><TableHead>User Info</TableHead><TableHead>Role</TableHead><TableHead>Status</TableHead><TableHead>Phone</TableHead><TableHead>Linked</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
                {loading ? <TableRow><TableCell colSpan={6} className="text-center py-8">Loading...</TableCell></TableRow> : filteredUsers.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center py-8">No users found.</TableCell></TableRow> : filteredUsers.map((user) => (
        ...
        {/* 2. Roles & Permissions Tab (UPDATED) */}
        <TabsContent value="roles" className="space-y-6">
            
            {/* Header / Actions */}
            <div className="flex justify-between items-center bg-white p-4 rounded-lg border shadow-sm">
                <div>
                    <h2 className="text-lg font-bold text-gray-800">Role Capabilities</h2>
                    <p className="text-sm text-gray-500">Configure what each role can access and perform.</p>
                </div>
                <div className="flex gap-3">
                    {isEditingRoles ? (
                        <>
                            <Button variant="outline" onClick={() => setIsEditingRoles(false)} className="text-red-600 border-red-200 hover:bg-red-50"><X className="h-4 w-4 mr-2"/> Cancel</Button>
                            <Button onClick={savePermissions} className="bg-green-600 hover:bg-green-700 text-white"><Save className="h-4 w-4 mr-2"/> Save Changes</Button>
                        </>
                    ) : (
                        <Button onClick={() => setIsEditingRoles(true)} className="bg-blue-600 hover:bg-blue-700 text-white"><Edit className="h-4 w-4 mr-2"/> Edit Permissions</Button>
                    )}
                </div>
            </div>
                    ...
                    <Card key={role} className={`border-t-4 ${role === 'client_admin' ? 'border-t-purple-600' : role === 'treasurer' ? 'border-t-green-600' : 'border-t-blue-600'}`}>
                        <CardHeader className="pb-2">
                            <div className="flex justify-between items-start">
                                <Badge className={getRoleBadgeColor(role)}>{role.replace('_', ' ').toUpperCase()}</Badge>
                                <span className="text-xs font-bold text-gray-400">{roleConfig[role].length} total</span>
                            </div>
                        </CardHeader>
                        <CardContent>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-gray-100">
                                ...
                                <TableHead className="text-center bg-purple-50 text-purple-900">Client Admin</TableHead>
                                <TableHead className="text-center bg-green-50 text-green-900">Treasurer</TableHead>
                                <TableHead className="text-center bg-blue-50 text-blue-900">Member</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                                        ...
                                        <TableRow key={perm}>
                                            ...
                                            <TableCell className="text-center">
                                                <div className="flex justify-center">
                                                    <Checkbox checked={roleConfig.client_admin.includes(perm)} disabled className="data-[state=checked]:bg-purple-600" />
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-center">
                                                <div className="flex justify-center">
                                                    <Checkbox 
                                                        checked={roleConfig.treasurer.includes(perm)} 
                                                        disabled={!isEditingRoles}
                                                        onCheckedChange={() => togglePermission('treasurer', perm)}
                                            ...
                                            <TableCell className="text-center">
                                                <div className="flex justify-center">
                                                    <Checkbox 
                                                        checked={roleConfig.member.includes(perm)} 
                                                        disabled // Usually members are read-only for security, but you can enable if needed: {!isEditingRoles}
            ...
            {isEditingRoles && (
                <Alert className="bg-yellow-50 border-yellow-200">
                    ...
                    <AlertDescription className="text-yellow-800 text-xs">
                        You are in Edit Mode. Changes to permissions will affect what users can see and do immediately after saving.
                    </AlertDescription>
                </Alert>
            )}
        </TabsContent>
