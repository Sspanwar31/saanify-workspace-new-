    ...
    <div className="relative min-h-screen overflow-hidden flex flex-col items-center justify-center p-4 bg-[#020617] font-poppins">
      ...
      {showTopBanner && (
        <div className="fixed top-0 left-0 w-full z-[1000] h-12 flex items-center justify-center px-4 gap-3 shadow-lg animate-in slide-in-from-top duration-700"
             ...
             <p className="text-white font-black text-xs md:text-sm uppercase tracking-[0.2em] text-center truncate max-w-[90vw]">{msgParts[0]}</p>
      ...
      <div className={`relative w-full max-w-[380px] transition-all duration-1000 ${isCardVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-90 translate-y-10 pointer-events-none'}`}>
        ...
        {/* 🚀 1. THEME OUTLINE (MAIN CONTAINER FIX) */}
        <div 
          className="relative bg-slate-950/90 backdrop-blur-3xl rounded-[3rem] shadow-[0_40px_100px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col border-[4px] transition-all duration-500"
          style={{ 
            // Ab poore card ki outline festival ke hisaab se hogi
            borderColor: isHoli ? '#ff0080' : themeColor,
            boxShadow: `0 0 50px ${themeColor}20` 
          }}
        >
          
          {/* 🚀 2. HERO SECTION (Golden Frame Removed) */}
          <div className="relative w-full h-[320px] overflow-hidden flex items-center justify-center bg-slate-950/50 p-6">
              
              {/* No more internal border-4 div here */}
              {broadcast.image_url ? (
                <img src={broadcast.image_url} className="hero-anim w-full h-full object-contain relative z-10 drop-shadow-2xl" alt="Festival Hero" />
              ) : (
              ...
              {/* Smooth Bottom Shadow for integration */}
          ...
          <div className="p-8 pt-0 text-center relative z-30 flex flex-col items-center">
            ...
            <div className="space-y-4 w-full px-2">
                ...
                <p className="text-slate-300 text-sm md:text-base font-medium leading-relaxed px-1 opacity-90">
                    {msgParts[0]}
                </p>
            </div>
      ...
      <style jsx global>{`
        ...
        .hero-anim { animation: hero-float ${broadcast?.hero_config?.speed || 4}s ease-in-out infinite; }
        @keyframes hero-float { 0%, 100% { transform: scale(1) translateY(0); } 50% { transform: scale(1.05) translateY(-8px); } }
      `}</style>
    </div>
  );
}
