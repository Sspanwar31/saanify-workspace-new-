...
export default function BroadcastPreviewPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden flex flex-col items-center justify-center p-4 sm:p-10 bg-[#020617]">
      
      {/* 🚀 TOP BANNER SIMULATION (Celebrate click ke baad dikhega) */}
      {showTopBanner && (
        <div className={`fixed top-0 left-0 w-full z-[10000] py-4 px-6 shadow-2xl animate-in slide-in-from-top duration-700 
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-center gap-3">
             ...
             <p className="text-base md:text-lg font-black italic tracking-wide drop-shadow-md">
      ...
      <div className="relative w-full max-w-[460px] bg-[#1a1a2e]/60 backdrop-blur-3xl border border-white/10 rounded-[4rem] shadow-[0_50px_150px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col items-center animate-in zoom-in-95 duration-700">
        ...
        {/* HERO SECTION (Image Fix: No Clipping) */}
        <div className="relative w-full h-[380px] overflow-hidden flex items-center justify-center">
            <img 
            ...
            {/* Modern Saanify Text on Image */}
            <div className="absolute top-10 w-full text-center px-4">
                <span className="text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)] uppercase tracking-[12px] font-serif text-xs font-black italic opacity-90">
                    S A A N I F Y
                </span>
            </div>
        </div>
        ...
        <div className="p-10 pt-0 text-center -mt-20 relative z-10 w-full space-y-8">
          ...
          {/* SAANIFY LUXURY LOGO BADGE (Globe hataya) */}
          <div className="w-24 h-24 mx-auto bg-gradient-to-br from-blue-600 to-indigo-800 rounded-[2.5rem] shadow-2xl flex items-center justify-center border-4 border-white/20 rotate-3 transform hover:rotate-0 transition-all duration-700">
             <ShieldCheck className="w-12 h-12 text-white" />
          </div>

          <div className="space-y-4 px-2">
            {/* Multi-Language Title (Modern Font & Spacing) */}
            <div className="flex flex-col gap-2">
                <h1 className="premium-text-effect text-5xl sm:text-6xl font-black uppercase tracking-tighter leading-none italic py-2">
                    {titleParts[0]}
                </h1>
            ...
            <div className="space-y-3 px-2">
                <p className="text-slate-200 text-base sm:text-lg font-bold leading-tight drop-shadow-sm">
                    {msgParts[0]}
                </p>
                {msgParts[1] && <p className="text-white/30 text-xs italic font-medium tracking-wide">{msgParts[1]}</p>}
            </div>
          </div>

          {/* CELEBRATE NOW BUTTON (Placed at Bottom) */}
          <Button 
            onClick={() => { setShowTopBanner(true); setTimeout(() => window.history.back(), 5000); }}
      ...
      <style jsx>{`
        h1 {
          font-family: 'Cinzel', serif; /* Branded font simulation */
        }
        .premium-text-effect {
          background: ${isHoli 
