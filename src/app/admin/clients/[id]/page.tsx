...
import { 
  ArrowLeft, ExternalLink, Mail, Phone, MoreVertical, 
  CreditCard, ShieldCheck, Users, TrendingUp, AlertTriangle, 
  CheckCircle, Trash2, Bell, FileText, Lock, RefreshCw, Calendar, Activity
} from 'lucide-react';
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 bg-slate-50 min-h-screen">
           ...
           <div className="flex gap-3 items-center">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="icon" className="rounded-full border-slate-300"><MoreVertical className="w-4 h-4 text-slate-600"/></Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel>Smart Actions</DropdownMenuLabel>
                  <DropdownMenuSeparator/>
                  <DropdownMenuItem onClick={() => handleAction('Notify')}><Bell className="mr-2 w-4 h-4 text-blue-500"/> Notify Client</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleAction('Ledger')}><FileText className="mr-2 w-4 h-4 text-purple-500"/> Statement & Ledger</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleAction('Renew')}><RefreshCw className="mr-2 w-4 h-4 text-green-500"/> Renew / Change Plan</DropdownMenuItem>
                  <DropdownMenuSeparator/>
                  <DropdownMenuItem onClick={() => handleAction('Lock')} className="text-orange-600"><Lock className="mr-2 w-4 h-4"/> Lock Account</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
           </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         ...
         <Card className="border-l-4 border-l-purple-500 shadow-sm hover:shadow-md transition-shadow">
            <CardContent className="p-6">
                <div className="flex justify-between items-start">
                    <div><p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Active Loans</p><h2 className="text-3xl font-bold text-slate-900 mt-2">89000</h2></div>
         ...
         <Card className="lg:col-span-2 shadow-sm border-slate-200 h-full">
            <CardHeader className="border-b border-slate-50 py-4 px-6">
                <div className="flex justify-between items-center">
                    <CardTitle className="flex items-center gap-2 text-base font-bold"><Calendar className="w-4 h-4 text-slate-500"/> Subscription Status</CardTitle>
                    <Button variant="outline" size="sm" className="h-8">Renew Plan</Button>
                </div>
            </CardHeader>
            <CardContent className="p-8">
               <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
                  ...
                  <div className="text-left md:text-right">
                      ...
                      <h4 className="text-xl font-bold text-slate-900">12/31/2025</h4>
                  </div>
               </div>
               <div>
                  <div className="flex justify-between text-xs mb-2 text-slate-500 font-medium"><span>Plan Usage</span><span>12 Days Remaining</span></div>
            ...
            <CardContent className="p-8 flex flex-col items-center justify-center">
               <div className="relative mb-4">
                   <div className="w-32 h-32 rounded-full border-[6px] border-slate-50 flex items-center justify-center">
                       <span className="text-4xl font-bold text-green-600">92<span className="text-xl text-slate-400 font-normal">/100</span></span>
                   </div>
               ...
               <div className="w-full mt-8 pt-6 border-t border-slate-50">
                  <div className="flex justify-between items-center">
                      <div><p className="text-xs text-slate-400 mb-1">This Month Revenue</p><p className="text-xl font-bold text-slate-900">₹125,000</p></div>
                      <div className="text-right"><TrendingUp className="w-6 h-6 text-green-500 ml-auto"/><p className="text-[10px] text-green-600 mt-1 font-bold">+12% growth</p></div>
                  </div>
               </div>
            </CardContent>
         </Card>
      ...
      <div className="border border-red-200 bg-red-50/50 rounded-xl p-6 flex flex-col md:flex-row justify-between items-center gap-4 hover:bg-red-50 transition-colors">
         <div>
            <h3 className="text-red-700 font-bold flex items-center gap-2 text-lg"><AlertTriangle className="w-5 h-5"/> Danger Zone</h3>
            <p className="text-sm text-red-600/80 mt-1">Irreversible actions. Deleting this client will remove all associated data including invoices and members.</p>
         </div>
         <Button onClick={handleDelete} className="bg-red-600 hover:bg-red-700 text-white border-none shadow-sm shadow-red-200 font-bold px-6">
