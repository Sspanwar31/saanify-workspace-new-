...
export function useReportLogic() {
  ...
  useEffect(() => {
    ...
    const totalExpenses = opsExpense;

    // Daily Ledger
    const ledgerMap = new Map();
    const getOrSetEntry = (dateStr: string) => {
        if (!ledgerMap.has(dateStr)) ledgerMap.set(dateStr, { date: dateStr, deposit: 0, emi: 0, loanOut: 0, interest: 0, fine: 0, cashIn: 0, cashOut: 0, cashInMode: 0, bankInMode: 0, upiInMode: 0, cashOutMode: 0, bankOutMode: 0, upiOutMode: 0 });
    ...
    passbookEntries.forEach(e => {
        ...
        else if (mode.includes('BANK')) bankBal += amt;
        else upiBal += amt;
    });
    ...
    const memberReports = members.map(m => {
        ...
        return { id: m.id, name: m.name, fatherName: m.phone, totalDeposits: dep, loanTaken: lTaken, principalPaid: lTaken - lPend, interestPaid: intPaid, finePaid, activeLoanBal: lPend, netWorth: dep - lPend, status: m.status };
    });
    ...
    const maturity = members.map(m => {
        ...
        return { memberName: m.name, joinDate: m.join_date || m.created_at, currentDeposit: Number(m.total_deposits || 0), targetDeposit: target, projectedInterest: projected, maturityAmount: maturityAmt, outstandingLoan: outstanding, netPayable: maturityAmt - outstanding };
    });
    ...
    const loansIssued = loans.reduce((acc, l) => acc + Number(l.amount || 0), 0);
    const loansPending = loans.reduce((acc, l) => acc + Number(l.remaining_balance || 0), 0);

    setAuditData({
        summary: { income: { interest: interestIncome, fine: fineIncome, other: otherIncome, total: totalIncome }, expenses: { ops: opsExpense, total: totalExpenses }, loans: { issued: loansIssued, recovered: loansIssued - loansPending, pending: loansPending }, netProfit: totalIncome - totalExpenses },
