  ...
  useEffect(() => {
    ...
    members.forEach(m => {
        ...
        if (mDepositEntries.length > 0) {
             ...
             const currentAccrued = monthlyShare * depositCount;
    ...
    // --- C. LOAN STATS (UPDATED: Calculate Principal from Passbook) ---
    const loansWithLiveBalance = loans.map(l => {
        // Find total installments paid for this member from Passbook
        ...
        return {
            ...
            principalPaid: installmentsPaid, // ✅ Calculated from Passbook Sum
            remaining_balance: currentBalance, // ✅ Calculated Live
            status: currentBalance <= 0 ? 'completed' : l.status // Auto-close if paid
        };
    });
